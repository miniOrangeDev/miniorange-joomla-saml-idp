...
class Miniorange_samlControllerMyaccount extends JControllerForm
{
    ...
    function requestForTrialPlan()
    {
        ...
        {
            $msg=($demo == 'Demo')? 'We have recieved your demo request. Someone from our company will contact you shortly regarding the next steps.':'We have recieved your trial request. Someone from our company will contact you shortly regarding the next steps.';
            $this->setRedirect('index.php?option=com_miniorange_saml&tab=request_demo', $msg);
        }
        else {
            $this->setRedirect('index.php?option=com_miniorange_saml&tab=request_demo', 'Server is busy. Please try again later.', 'error');
            return;
        }
    ...
    function verifyCustomer()
    {
        ...
        if (strcasecmp($customerKey['apiKey'], 'CURL_ERROR') == 0) {
            ...
            $this->saveSuccessCustomer($customerKey, 'Your account has been retrieved successfully, now you can <strong>upgrade</strong> to licensed version.', $email);
        } else {
    ...
    function save_success_customer_config($email, $id, $apiKey, $token, $phone, $msg)
    {
        ...
        $result = new Mo_saml_Local_Util();
        $result->generic_update_query($database_name, $updatefieldsarray);
        $erMsg = 'Your account has been retrieved successfully, now you can <strong>upgrade</strong> to licensed version.';
    ...
    function ResetAccount()
    {
        ...
        $result = new Mo_saml_Local_Util();
        $result->generic_update_query($database_name, $updatefieldsarray);
        $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'The account has been successfully removed from the plugin.');
        return;
    }
    ...
    function upload_metadata($file)
    {
        ...
        if (!empty($first_child)) {
            ...
            $this->setRedirect('index.php?option=com_miniorange_saml&tab=idp', 'Identity Provider details saved successfully. Please click on <a href="index.php?option=com_miniorange_saml&tab=idp&test-config=true" class="mo_boot_btn mo_boot_btn-saml">Test Configuration</a> button to proceed further.');
            return;
        } else {
            $this->setRedirect('index.php?option=com_miniorange_saml&tab=idp', 'Please provide valid metadata.', 'error');
            return;
    ...
    function importexport()
    {
        ...
        {
            $this->setRedirect('index.php??morequest=acs', 'Downloded sucessfully the IDP configuration.');
            return;
        }
    ...
    function saveAdminMail()
    {
        ...
        $db->setQuery($query);
        $result = $db->execute();
        $this->setRedirect('index.php?option=com_miniorange_saml&tab=com_miniorange_saml&tab=idp','Email successfully changed');
        return;
    }
    ...
    function saveConfig()
    {
        ...
        if ($updated_settings == "mo_saml_save_config") {
            ...
            if ($upload == "upload_cert") {
                if ($_FILES["myFile"]["name"] != "") {
                    ...
                    if ($cer_var[1] == 'cer' || $cer_var[1] == 'cert' || $cer_var[1] == 'crt') {
                        ...
                        $message = 'Please upload IdP certificate file with correct extension such as .crt, .cert, .cer.';
                        $this->setRedirect('index.php?option=com_miniorange_saml&tab=idp', $message, 'error');
                        return;
                    }
                } else {
                    $message = 'Please select certificate file of IdP to upload.';
                    $this->setRedirect('index.php?option=com_miniorange_saml&tab=idp', $message, 'error');
                    return;
                }
            } else {
            ...
            //Save saml configuration
            $message = 'Your configuration has been saved successfully. Please click on <a href="index.php?option=com_miniorange_saml&tab=idp&test-config=true" class="mo_boot_btn mo_boot_btn-saml">Test Configuration</a> button to proceed futher.';
            $status = 'success';
        ...
        if (strcmp($password, $confirmPassword) == 0) {
            ...
            if (strcasecmp($content['status'], 'CUSTOMER_NOT_FOUND') == 0) {
                ...
                if (strcasecmp($content['status'], 'SUCCESS') == 0) {
                    ...
                    $proxy_setup = new Mo_saml_Local_Util();
                    $proxy_setup->generic_update_query($database_name, $updatefieldsarray);
                    $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'A One Time Passcode(OTP) has been sent to <strong>' . $email . '</strong>. Please enter the OTP to verify your email. ');
                } else {
                    $database_name = '#__miniorange_saml_customer_details';
                    $updatefieldsarray = array(
    ...
    function validateOtp()
    {
        ...
        if (strcasecmp($content['status'], 'SUCCESS') == 0) {
            ...
            if (strcasecmp($customerKey['status'], 'CUSTOMER_USERNAME_ALREADY_EXISTS') == 0) {    //admin already exists in miniOrange
                ...
                $proxy_setup = new Mo_saml_Local_Util();
                $proxy_setup->generic_update_query($database_name, $updatefieldsarray);
                $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'There was an error creating an account for you. You may have entered an invalid Email-Id. <br><strong>(We discourage the use of disposable emails)</strong><br>
            ...
            $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'There was an error creating an account for you.
                            You may have entered an invalid Email-Id. <br><strong>(We discourage the use of disposable emails)</strong><br>Please go back and try again with a valid email.', 'error');
            return;
    ...
    function resendOtp()
    {
        ...
        if (strcasecmp($content['status'], 'SUCCESS') == 0) {
            ...
            if ($email_count != '') {
                ...
                $proxy_setup = new Mo_saml_Local_Util();
                $proxy_setup->generic_update_query($database_name, $updatefieldsarray);
                $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'Another One Time Passcode has been sent to <strong>' . ($admin_email) . '</strong>. Please enter the OTP below to verify your email.');
            } else {
                $database_name = '#__miniorange_saml_customer_details';
                $updatefieldsarray = array('registration_status' => 'MO_OTP_DELIVERED_SUCCESS',);
                $proxy_setup = new Mo_saml_Local_Util();
                $proxy_setup->generic_update_query($database_name, $updatefieldsarray);
                $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'An OTP has been sent to <strong>' . ($admin_email) . '</strong>. Please enter the OTP below to verify your email.');
            }
        } else if (strcasecmp($content['status'], 'CURL_ERROR') == 0) {
            $database_name = '#__miniorange_saml_customer_details';
        ...
        if (preg_match($pattern, $phone, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            if ($send_otp_response->status == 'SUCCESS') {
                ...
                if ($sms_count != '') {
                    ...
                    $proxy_setup = new Mo_saml_Local_Util();
                    $proxy_setup->generic_update_query($database_name, $updatefieldsarray);
                    $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'Another One Time Passcode has been sent <strong>(' . $sms_count . ')</strong> for verification to ' . $phone);
                } else {
                    $database_name = '#__miniorange_saml_customer_details';
                    ...
                    $proxy_setup = new Mo_saml_Local_Util();
                    $proxy_setup->generic_update_query($database_name, $updatefieldsarray);
                    $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'One Time Passcode has been sent ( <strong>1</strong> ) for verification to ' . $phone);
                }
            }
        } else {
            $this->setRedirect('index.php?option=com_miniorange_saml&tab=account', 'Please enter the phone number in the following format: <strong>+##country code## ##phone number##', 'error');
            return;
        }
    }
